...
const PayoutSetup = (props) => {
	...
	const onSubmit = async (event, merchantStripeId) => {
		...
		if (localStorage.getItem('business')) {
			const currentBusiness = new Business(localStorage.getItem('business'), true);
			setLocalStorage('business', currentBusiness);

			const dataObject = { business: currentBusiness };
			let result = await API.makeRequest('POST', '/signup-redirect', dataObject, false);
	...
	if (props.callback) {
		return (
			<>
				<div className='text-center box'>
					...
					<h5 style={{ marginTop: '5%', marginBottom: '5%' }}>Stripe account onboarding incomplete</h5>
					<p>Please click the button below to be redirected to Stripe to complete the onboarding proces</p>
		...
		return (
			<>
				<div className='text-center box'>
					<div className='fs-title'>Input your bank account to recieve payouts</div>
					<img src={bankIcon} alt='' className='icon' />

					<p>QuickBev partners with Stripe to transfer earnings to your bank account.</p>

					<Button
						className='btn btn-primary text-center'
						onClick={(event) => {
							handleConnect().then((merchantStripeId) =>
								props.onSubmit(event, merchantStripeId, false).then((result) => {
									setRedirect(redirectUrl);
								})
							);
						}}
